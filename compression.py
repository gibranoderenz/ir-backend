...
class VBEPostings:
    ...
    def encode(postings_list):
        ...
        bytes
            bytearray yang merepresentasikan urutan integer di postings_list
        """
        gap_based_list = copy.deepcopy(postings_list)
        for i in range(1, len(gap_based_list)):
            gap_based_list[i] = postings_list[i] - postings_list[i - 1]
        vb_encoded_list = VBEPostings.vb_encode(gap_based_list)
        return array.array('B', vb_encoded_list).tobytes()
    ...
    def encode_tf(tf_list):
        ...
        bytes
            ...
            dokumen pada list of postings
        """
        encoded_tf_list = VBEPostings.vb_encode(tf_list)
        return array.array('B', encoded_tf_list).tobytes()
    ...
    def decode(encoded_postings_list):
        ...
        List[int]
            list of docIDs yang merupakan hasil decoding dari encoded_postings_list
        """
        temp_list = VBEPostings.vb_decode(encoded_postings_list)
