...
class IdMap:
    ...
    def __getitem__(self, key):
        ...
        """
        # Asumsi: key dijamin berupa string atau integer
...
def merge_and_sort_posts_and_tfs(posts_tfs1, posts_tfs2):
    ...
    """
    # Ide: https://stackoverflow.com/a/54228150
    result = {}
    merged_list = posts_tfs1 + posts_tfs2

    for doc_id, tf in merged_list:
        result[doc_id] = result.get(doc_id, 0) + tf

    sorted_list = sorted(list(result.items()), key=lambda x:x[0])
    return sorted_list
